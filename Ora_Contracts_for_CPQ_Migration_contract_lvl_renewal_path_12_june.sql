...
--Updated 6/7/24 to update case statement for Student Count to account for Student Count of 0 (zero)
    ...
    from
        (select --count(distinct ch.contract_number),
            ...
            --this is the list of customers from Michele T that need to be set up for "full touch" in CPQ renewals 
                 case when (qte.quoted_cust_num = ca.account_number OR ca.account_number in 
                 ...
                 round(case when opa.pricing_context = 'FSC_PER_PUPIL_PRICING'
                            ...
                            when opa.pricing_context = 'FSC_PER_PUPIL_PRICING'
                              and opa.pricing_attribute1 is not null
                              and opa.pricing_attribute1 = 0
                            then 0
                            else NULL
            ...
            from okc.okc_k_headers_all_b ch,
                 ...
                 (select c.*
                    from 
                        (select q.company, w.parent_account_number parent_num, pp.party_name parent_name,  
                               q.owner_account_number owner_num, p.party_name owner_name,
                               sp.party_number quoted_cust_num, sp.party_name quoted_cust_name, to_char(w.warr_end, 'YYYY-MM') service_end,
                               to_char(q.new_exprn_date,'YYYY-MM') new_service_end, q.service_duration_quantity num_of_months, 
                               w.renewal_amount current_amt, q.hdr_qte quote, q.runid, q.hdr_created_date quote_date, 
                               q.quote_expires_on,
                               --decode(q.hdr_created_by,'ANONYMOUS','SYSTEM','FSCPROC','SYSTEM',q.hdr_created_by) 
                               q.hdr_created_by quote_created_by,
                               q.itemno, q.description, q.prodline, q.prodfamily, q.instance_id, w.instance_status,
                               q.annual_qted_price,
                               round(case when q.annual_qted_price = 0
                                    then 0
                                    when w.renewal_amount = 0
                                      and q.annual_qted_price > 0
                                    then 100
                                    else ((q.annual_qted_price-w.renewal_amount)/w.renewal_amount)*100
                               end,2) uplift_pct,
                               row_number () over (partition by sp.party_number, q.instance_id order by q.hdr_qte desc) rn
                            from fscstd.service_warranty w,
                                 ar.hz_parties pp,
                                 ar.hz_parties p,
                                 ar.hz_parties sp,
                                (select qh.quote hdr_qte, trunc(qh.creation_date) hdr_created_date, qh.soldto_account_number, usr.user_name hdr_created_by, 
                                        ql.current_exprn_date, ql.runid, ql.status qte_line_status, ql.owner_account_number, pt.itemno, pt.description, 
                                        decode(pt.prodline, 'SUPPORT-SW',pt.prodfamily, pt.prodline) prodline,
                                        decode(pt.prodline, 'SUPPORT-SW',pt.prodgroup,pt.prodfamily) prodfamily, 
                                        qh.quote_valid_upto quote_expires_on, ql.instance_id, 
                                        round(case when ql.service_duration_quantity = 12
                                             then ql.selling_price
                                             else (ql.selling_price/ql.service_duration_quantity)*12
                                        end,2) annual_qted_price,
                                        --ql.selling_price, 
                                        ql.service_duration_quantity,
                                        ql.new_exprn_date,
                                        ql.rql_status,
                                        decode(qh.org_id,83,'FSS','X2') company
                                    from fscstd.quote_hdrs_stg qh,
                                         fscstd.quote_lines_stg ql,
                                         fscstd.fss_product_tree pt,
                                         applsys.fnd_user usr
                                    where qh.status = 'NEW'
                                      and qh.quote = ql.quote
                                      and ql.current_exprn_date >= add_months(trunc(sysdate,'month'),-3)
                                      and ql.service_duration_quantity != 0
                                      and pt.prodtree = 'STANDARD'
                                      and pt.platform != 'XOLS'
                                      and pt.inventory_item_id = ql.support_inventory_item_id
                                      and pt.org_id = qh.org_id
                                      and usr.user_id = qh.created_by) q
                            where w.warr_end >= add_months(trunc(sysdate,'month'),-3)
                              and w.parent_account_number = pp.party_number
                              and pp.party_number = p.attribute7
                              and p.party_id = w.party_id
                              and sp.party_number = q.soldto_account_number
                              --and sp.party_number = '2002528'  --used for validation
                              and q.owner_account_number = p.party_number
                              and q.instance_id = w.instance_id
                              and q.current_exprn_date = w.warr_end
                              --and decode(q.hdr_created_by,'ANONYMOUS','SYSTEM','FSCPROC','SYSTEM',q.hdr_created_by) = 'SYSTEM' 
                              ) c
                    where rn = 1 ) qte
            where 1 = 1 
              ...
              and qte.quoted_cust_num(+) = ca.account_number
              and qte.owner_num(+) = op.party_number
              and qte.instance_id(+) = ci.object1_id1
--these conditions are here to look for different selling scenarios
              ...
              --and pp.party_number = '3110550'
              --and op.party_number = '3602814'
              --and ca.account_number = '3110550' 
...
--                       else ol.ordered_item
--                  end = '95201P'
--              and decode(pt.prodline,'SUPPORT-SW',pt.prodfamily,pt.prodline) = 'ACCESSIT'
              --and decode(pt.prodline,'SUPPORT-SW',pt.prodgroup,pt.prodfamily) like '% SCHOOL'
              ) con
...
order by --support_status --this is here to get the distinct count of contracts by status
         con.contract_id --con.parent_account_number, con.account_number, con.description
;
